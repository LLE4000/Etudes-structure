...
# ----------------------------
# Données cornières standards
# (liste compacte issue de ton tableau – à compléter si besoin)
...
with left:
    ...
    st.header("Charges")
    charge_type = st.radio("Type de charge", ["Linéaire (kN/m)", "Ponctuelle (kN)"], horizontal=True)
    if charge_type.startswith("Linéaire"):
        V_char = st.number_input("Effort tranchant V (kN/m)", min_value=0.0, value=5.0, step=0.1)
        V_unit = "kN/m"
    else:
        V_char = st.number_input("Effort tranchant V (kN)", min_value=0.0, value=5.0, step=0.1)
        V_unit = "kN"

    e = st.number_input("Excentricité e (cm) – distance charge/cornière", min_value=0.0, value=2.3, step=0.1)
    gamma_ELU = st.number_input("Coefficient global (ELU)", min_value=1.0, value=1.35, step=0.05)

    st.caption("💡 Le moment transversal est pris **M = V × e** (avec V en kN par mètre de cornière considérée et e en m).")

    st.divider()

    st.header("Ancrages")
    s = st.number_input("Espacement des fixations s (cm)", min_value=10.0, value=30.0, step=1.0)
    edge = st.number_input("Distance de l’âme au bord du mur (cm)", min_value=0.0, value=2.0, step=0.5)

    st.divider()

    st.header("Critères matériaux (admissibles)")
    st.caption("Tu peux reprendre tes valeurs ELS/ELU de la note. Par défaut : acier S235 (ordres de grandeur).")
    sig_lim_ELS = st.number_input("σ_lim, ELS (N/mm²)", min_value=10.0, value=155.0, step=5.0)
    tau_lim_ELS = st.number_input("τ_lim, ELS (N/mm²)", min_value=0.1, value=0.8, step=0.1)
    sig_lim_ELU = st.number_input("σ_eq lim, ELU (N/mm²)", min_value=10.0, value=186.0, step=5.0)
...
# Choix "b" pour le modèle lame
# b = 1000 mm pour une vérif par mètre courant (comme dans ta note)
b_trans_mm = 1000.0
t_mm = float(t)
...
if V_unit == "kN/m":
    ...
    # Ponctuelle: on ramène par mètre de cornière pour le check transversal
    # Hypothèse simple: on saisit la longueur d’influence L (m)
    L = st.sidebar.number_input("Longueur d’influence L pour charge ponctuelle (m)", 0.1, 5.0, 1.0, step=0.1)
    V_ELS = V_char / L           # kN/m
    V_ELU = (V_char * gamma_ELU) / L

# Moment transversal par mètre (kN·m/m) -> convertir en N·mm/mm
e_m = to_m(e)
M_ELS_Nmm = V_ELS * e_m * 1e6   # (kN/m * m) * 1e6 = N·mm/mm
M_ELU_Nmm = V_ELU * e_m * 1e6

# Contraintes transversales
sigma_ELS = M_ELS_Nmm / W_trans if W_trans > 0 else 0.0       # N/mm²
sigma_ELU = M_ELU_Nmm / W_trans if W_trans > 0 else 0.0
tau_ELS   = (V_ELS * 1e3) / Az_trans if Az_trans > 0 else 0.0 # (kN/m -> N/mm) / mm² = N/mm²
...
# ----------------------------------------------------------
# Structure en place + TODO clair pour brancher exactement ta formule.
# Ici on prend une "lame" de largeur b = s (mm) chargée par la résultante locale V_loc = V * s/1000 (kN)
# et on calcule à titre indicatif M_long = V_loc * e  (à adapter si tu considères M = q*s^2/8, etc.)
b_long_mm = to_mm(s)            # mm
I_long = b_long_mm * t_mm**3 / 12.0
W_long = b_long_mm * t_mm**2 / 6.0
Az_long = b_long_mm * t_mm

Vloc_ELS = V_ELS * (s/100.0)    # kN (par panneau entre deux fixations)
Vloc_ELU = V_ELU * (s/100.0)    # kN

# >>> TODO: si tu veux utiliser "poutre simplement appuyée" sous charge répartie q = V_ELS (kN/m),
# alors M_max = q * (s/100)**2 / 8  (kN·m). Remplace la ligne suivante par cette formule si c’est ton cas.
...
with right:
    ...
    st.markdown(
        f"""
- **Efforts** :  
  • V (ELS) = **{V_ELS:.2f} {V_unit}** → V (ELU) = **{V_ELU:.2f} {V_unit}**  
...
"""
    ...
    st.caption("Hypothèse simple par panneau de largeur s. Remplace **M_long** dans le code par ta formule exacte (TODO).")
...
"""
    ...
    st.caption("À compléter selon ton logiciel/ETA (ex. HILTI) : cisaillement/traction, entraxe, profondeur, béton, etc.")
    st.write("- Calculer V/anchorage, N/anchorage, interaction, et comparer aux résistances de l’ETA.")
    st.write("- Vérifier les distances au bord et l’épaisseur de support.")

    st.divider()
    st.subheader("Notes & hypothèses")
    st.markdown(
        """
- Modèle **lame** (par mètre) conforme à ta fiche : \(I=b t^3/12,\ v=t/2,\ W=b t^2/6,\ A_z=b t\).  
- **Transversal** : \(M = V \times e\).  
- **Longitudinal** : panneau de largeur **s**. **Remplacer la formule de M_long** par ta relation exacte (poutre entre fixations, combinaison cornière+charge).  
- Critères **ELS/ELU** paramétrables.  
- Ajouter ensuite le module **ancrages** (ETA) suivant ta bibliothèque interne.
"""
    )
