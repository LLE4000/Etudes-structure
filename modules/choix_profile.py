...
@st.cache_data def load_profiles(): with open("profiles_test.json") as f: return json.load(f)

def calcul_contraintes(profile, M, V, fyk): Wel = profile["Wel"] * 1e3  # cm3 -> mm3 Avz = profile["Avz"] * 1e2  # cm2 -> mm2
...
def dessiner_profile(profile): h, b, tf, tw, r = profile["h"], profile["b"], profile["tf"], profile["tw"], profile.get("r", 0) scale = 0.5 h *= scale b *= scale tf *= scale tw *= scale r *= scale
...
if M > 0 and V > 0:
    ...
    for name, prof in profiles.items():
        ...
        resultats.append({
            ...
            "Type": prof["type"],
            "h": int(round(prof["h"])),
            "Wel": int(round(prof["Wel"])),
            "Avz": int(round(prof["Avz"])),
            "Iv": int(round(prof["Iv"])),
            "Poids [kg/m]": int(round(prof["G"])),
    ...
    if st.checkbox("Afficher tous les profilés ✔/❌"):
        def surligner(row):
            utilisation = float(row["Utilisation"].rstrip('%'))
            couleur = "background-color: #d4f7d4" if utilisation <= 100 else "background-color: #f7c6c6"
