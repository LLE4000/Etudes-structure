...
- Mise en page compacte pour tenir sur 1 page A4
- Formules LaTeX: uniquement la version NUMÉRIQUE (valeurs intégrées)
- Images d'équations ~×3 plus petites
- Conclusions de choix (As et étriers) avec statut OK/Non
"""
...
def render_equation(tex_expr, out_path, fontsize=12, pad=0.06):  # ★ fontsize↓ pad↓
...
def get_styles():
    ...
    styles = {
        "Title": ParagraphStyle("Title", parent=ss["Title"],
                                fontName=base, fontSize=16, leading=19,  # ★ plus compact
        ...
        "EqTitle": ParagraphStyle("EqTitle", parent=ss["BodyText"],
                                  fontName=base, fontSize=9.5, leading=12,  # ★
                                  textColor=colors.HexColor("#666"), spaceAfter=1),
...
# ---------------------------------------------------------------------
# Blocs équations (NUMÉRIQUE SEULEMENT)
# ---------------------------------------------------------------------

def eq_block(eq_no, tmpdir, tex_numeric, result_text=None, img_width_mm=40):  # ★ img_width 40 mm
    """Bloc équation compact: titre + image numérique (valeurs) + résultat."""
    S = get_styles()
    flows = [Paragraph(f"<b>Équation ({eq_no})</b>", S["EqTitle"])]

    pth_num = os.path.join(tmpdir, f"eq_num_{eq_no}.png")
    render_equation(tex_numeric, pth_num, fontsize=12)
    flows.append(Image(pth_num, width=img_width_mm*mm, hAlign="LEFT"))

    if result_text:
        flows.append(Spacer(1, 1.0*mm))
        flows.append(Paragraph(result_text, S["EqResult"]))

    flows.append(Spacer(1, 3*mm))  # ★ compact
    return flows
...
def aire_barres(n, phi):
    return float(n) * math.pi * (float(phi)/2.0)**2  # mm²
...
def generer_rapport_pdf(
    ...
    V=0.0,
    V_lim=0.0,
    # ★ nouveaux paramètres pour afficher les CHOIX du programme
    ...
    **kwargs,
    ...
    try:
        doc = SimpleDocTemplate(
            out_path,
            pagesize=A4,
            leftMargin=14*mm, rightMargin=14*mm,   # ★ marges réduites
        ...
        # Armatures inférieures (numérique + conclusion de CHOIX)
        ...
        res_as = f"Résultat : <b>A<sub>s,inf</sub> = {fr(As_inf_req,1)} mm²</b>"
        flow += eq_block(2, tmpdir, tex_as_num, res_as)
