...
def eq_block(eq_no, tmpdir, tex_numeric, text_below=None, img_width_mm=38):
    """
    Image LaTeX NUMÉRIQUE proportionnelle + petit texte dessous (sans 'Résultat :').
    img_width_mm = 32 * 1.2  (≈ +20%)
    """
    S = get_styles()
    flows = []
    ...
    if text_below:
        ...
        flows.append(Paragraph(text_below, S["Eq"]))
...
def generer_rapport_pdf(
    ...
    **kwargs,
    ...
    try:
        ...
        # --- En-tête projet (sans grand titre)
        flow.append(Table(
            [
        ...
        flow += eq_block(1, tmpdir, tex_hmin_num, text_below=f"h_min = {fr(hmin,1)} cm")
        ok_h = (hmin + enrobage) <= h
        lim_line = f"h_min + enrobage = {fr(hmin+enrobage,1)} cm ≤ h = {fr(h,1)} cm — {verdict(ok_h)}"
        flow.append(Paragraph(lim_line, S["Green" if ok_h else "Red"]))
        flow.append(Spacer(1, 2*mm))

        # --- Calcul des armatures
        flow.append(Paragraph("Calcul des armatures", S["H1"]))
        ...
        flow += eq_block(2, tmpdir, tex_as_inf_num, text_below=f"A_s,inf = {fr(As_inf_req,1)} mm²")
        flow.append(Paragraph(
            f"A_s,min = {fr(As_min,1)} mm² ; A_s,max = {fr(As_max,1)} mm²",
            S["Small"])
        )
        ...
        if M_sup and M_sup > 0:
            ...
            flow += eq_block(3, tmpdir, tex_as_sup_num, text_below=f"A_s,sup = {fr(As_sup_req,1)} mm²")
            flow.append(Paragraph(
                f"A_s,min = {fr(As_min,1)} mm² ; A_s,max = {fr(As_max,1)} mm²",
                S["Small"])
            )
        ...
        if V and V > 0:
            ...
            flow += eq_block(4, tmpdir, tex_tau_num, text_below=f"τ = {fr(tau,2)} N/mm²")

            # Comparaison directe au seuil pertinent
            if tau <= tau_1:
                lim_lab, lim_val = "τ_adm I", tau_1
                ok_tau = True
            elif tau <= tau_2:
                lim_lab, lim_val = "τ_adm II", tau_2
                ok_tau = True
            elif tau <= tau_4:
                lim_lab, lim_val = "τ_adm IV", tau_4
                ok_tau = True
            else:
                lim_lab, lim_val = "τ_adm IV", tau_4
                ok_tau = False

            comp_line = (f"τ = {fr(tau,2)} N/mm² < {lim_lab} = {fr(lim_val,2)} N/mm² — {verdict(ok_tau)}"
                         if ok_tau else
                         f"τ = {fr(tau,2)} N/mm² > {lim_lab} = {fr(lim_val,2)} N/mm² — {verdict(False)}")
            flow.append(Paragraph(comp_line, S["Green" if ok_tau else "Red"]))
            flow.append(Spacer(1, 2*mm))

            # --- Calcul des étriers
            flow.append(Paragraph("Calcul des étriers", S["H1"]))
            ...
            flow += eq_block(5, tmpdir, tex_s_num, text_below=f"s_th = {fr(s_th,1)} cm")
