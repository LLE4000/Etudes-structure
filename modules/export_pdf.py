...
def generer_rapport_pdf(nom_projet, partie, date, indice, beton, fyk, b, h, enrobage, M_inf, M_sup, V, V_lim):
    ...
    try:
        ...
        elements.append(Paragraph(f"Erreur dans la vérification de hauteur utile : {e}", styles['Note']))
        elements.append(Spacer(1, 12))
    ...
    try:
        ...
        elements.append(Paragraph(f"Erreur dans la vérification cisaillement : {e}", styles['Note']))
        elements.append(Spacer(1, 12))

    # Détermination des étriers
    try:
        elements.append(Paragraph("Détermination des étriers", styles['TitreSection']))
        Ast = 2 * (3.14 * (8 / 2) ** 2)  # 2Ø8 par défaut
        fyd = float(fyk) / 1.5 if str(fyk).isdigit() else 0
        d = h - enrobage
        ...
        elements.append(Paragraph(f"Erreur dans le calcul des étriers : {e}", styles['Note']))
        elements.append(Spacer(1, 12))
